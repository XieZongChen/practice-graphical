...
注 1：**变换矩阵应用的结果并不直接影响每个像素的计算，而是影响整个元素的合成层，这样浏览器可以将变换应用于该合成层，而不必重新计算整个页面的布局**。

**矩阵变换计算过程**（md 查看软件需要支持 LaTeX）：
...
在 3D 空间中，齐次坐标的形式为 `(x, y, z, w)`。`(x, y, z)` 是三维空间中的笛卡尔坐标。`w` 是用于透视投影的齐次因子，通常为 1，但在某些透视变换中可能不是 1。
